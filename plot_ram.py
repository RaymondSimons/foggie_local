...
for sat in arange(1):
    ...
    for s, simname in enumerate(simnames):
        ...
        gd = where((~isnan(data.data['density_02'])) & (td < tmax[sat]))[0]
        ...
        for p, perc in enumerate(['02', '16', '50', '84', '98']):
            ...
            ax2.plot(td[gd], data.data['z_velocity_%s'%perc][gd], color = clrs[s],  linewidth = lws[p], linestyle = ls[p], alpha = lalp[p])
            ax3.plot(td[gd], data.data['z_velocity_%s'%perc][gd]**2. * data.data['density_%s'%perc][gd], color = clrs[s],  linewidth = lws[p], linestyle = ls[p], alpha = lalp[p])
    ...
    for ax in [ax1, ax2, ax3]:
        ...
        ax.annotate('Satellite %i'%sat, (0.05, 0.05), xycoords = 'axes fraction', fontsize = 16)
    if sat%3 == 0:
