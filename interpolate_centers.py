...
for s, simname in enumerate(simnames):
    ...
    for st in arange(6):
        ...
        for d, DD in enumerate(DD_arr):
            ...
            if os.path.isfile(fname):
                ...
                if len(data['SAT_%.2i'%st].data['box_avg']) > 0:
                    DDs.append(DD)
                    xs.append(data['SAT_%.2i'%st].data['box_avg'][0])
                    ys.append(data['SAT_%.2i'%st].data['box_avg'][1])
                    zs.append(data['SAT_%.2i'%st].data['box_avg'][2])
        ...
        coord_datas_all['SAT_%.2i'%st] = dic_2
        ...
        axes[st,0].annotate('SAT %.2i'%st, (0.1, 0.9), va = 'top', xycoords = 'axes fraction', fontsize = 20)
