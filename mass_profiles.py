...
for s, simname in enumerate(['nref11n_nref10f_selfshield_z6']):#, 'nref11n_selfshield_z15']):
    DDmin = 660
    DDmax = 1050
    #if s == 1 : DDmax = 1050
    #else:       DDmax = 900
    ...
    for sat_n in arange(5):
        ...
        for d, DD in enumerate(arange(DDmin+1, DDmax, 1)):
            ...
            data_p = fits.open(fits_dir + '/%s_DD%.4i_mass_sat%.2i_1049.fits'%(simname, DD-1, sat_n))
            data   = fits.open(fits_dir + '/%s_DD%.4i_mass_sat%.2i_1049.fits'%(simname, DD, sat_n))
        ...
        fs = 25
        if s == 0:
            axes2.annotate('R = 0.4 kpc', (1.52, 1.7 * sm1[0]), color = 'black',  fontweight = 'bold', fontsize = fs, zorder = 10)
            axes2.annotate('1 kpc',       (1.52, 1.7 * sm2[0]), color = 'black',  fontweight = 'bold', fontsize = fs, zorder = 10)
            axes2.annotate('3 kpc',       (1.52, 1.7 * sm3[0]), color = 'black',  fontweight = 'bold', fontsize = fs, zorder = 10)
        ...
        if True:
            for d, DD in enumerate(arange(DDmin+1, DDmax, 1)):
                ...
                figxi.savefig(fig_dir + '/masses_time_DD/%s_masses_time_sat%.2i_DD%.4i.png'%(simname, sat_n, DD), dpi = 300)
